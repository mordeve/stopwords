...
func CleanString(content string, stopwordsJsonMap map[string]interface{}, cleanHTML bool) string {
	return string(Clean([]byte(content), stopwordsJsonMap, cleanHTML))
}

// Clean removes useless spaces and stop words from a byte slice.
// BCP 47 or ISO 639-1 language code (if unknown, we'll apply english filters).
// If cleanHTML is TRUE, remove HTML tags from content and unescape HTML entities.
func Clean(content []byte, stopwordsJsonMap map[string]interface{}, cleanHTML bool) []byte {
	//Remove HTML tags
	if cleanHTML {
	...
	content = removeStopWords(content, stopwordsJsonMap)
...
func removeStopWords(content []byte, dict map[string]interface{}) []byte {
	var result []byte
